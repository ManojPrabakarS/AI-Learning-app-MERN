...
const QuizCard = ({ quiz, onDelete }) => {
    return (
        <div className='group relative bg-white/80 backdrop-blur-xl border-2 border-slate-200 hover:border-purple-300 rounded-2xl p-4 transition-all duration-200 hover:shadow-purple-500/10 flex flex-col justify-between'>
            <button onClick={(e) => {
                e.stopPropagation();
                onDelete(quiz);
            }}
            ...
            <div className='space-y-4'>
                ...
                <div className='flex items-center gap-3 pt-2 border-slate-100'>
                    <div className='px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg'>
                        <span className='text-sm font-semibold text-slate-700'>{quiz.questions.length}{" "}
                ...
                {quiz?.userAnswers?.length > 0 ? (
                    ...
                    (<>
                        <Link to={`/quizzes/${quiz._id}`}>
                            <button className='group/btn relative w-full h-11 bg-linear-to-r from-purple-500 to-violet-500 hover:from-purple-600 hover:to-violet-600 text-white font-semibold text-sm rounded-xl transition-all duration-200 shadow-lg shadow-purple-500/25 active:scale-95 overflow-hidden'>
