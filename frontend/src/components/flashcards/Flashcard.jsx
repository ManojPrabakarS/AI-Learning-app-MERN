            ...
            >
                ...
                <div className='absolute inset-0 w-full h-full bg-white/80 backdrop-blur-xl border-2 border-slate-200/60 rounded-2xl shadow-xl shadow-slate-200/50 p-8 flex flex-col justify-between' style={{ backfaceVisibility: 'hidden', WebkitBackfaceVisibility: 'hidden' }}>
                    ...
                    <div className='flex items-start justify-between'>
                        <div className='bg-slate-200 text-[10px] text-slate-600 font-medium rounded px-4 py-1 uppercase'>{flashcard?.difficulty}</div>
                        <button
                            onClick={(e) => {
                                e.stopPropagation();
                            ...
                            className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all duration-200 ${flashcard.isStarred ? 'bg-linear-to-br from-amber-300 to-yellow-500 text-white shadow-lg shadow-amber-500/25'
