...
const FlashcardManager = ({ documentId }) => {
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            ...
            toast.error(error.message || 'Failed to delete Card set')
        } finally {
            setDeleting(false)
        }
    };
        ...
        if (flashcardSets.length === 0) {
            return (
                <div className='flex flex-col items-center justify-center py-16 px-6'>
                    <div className='inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-linear-to-br from-purple-100 to-violet-100 mb-6'>
                        <Puzzle className='w-8 h-8 text-purple-600' strokeWidth={2} />
                    </div>
                ...
                <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4'>
                    {flashcardSets.map((set) => (
                        <div
                            ...
                            className='group relative bg-white/80 backdrop-blur-xl border-2 border-slate-200 hover:border-purple-300 rounded-2xl p-6 cursor-pointer transition-all duration-200 hover:shadow-lg hover:shadow-purple-500/10'
                        >
                            {/* Delete Button */}
                            <button
                            ...
                            <div className='space-y-4'>
                                <div className='inline-flex items-center justify-center w-12 h-12 rounded-xl bg-linear-to-br from-purple-100 to-violet-100'>
                                    <Album className='w-6 h-6 text-purple-600' strokeWidth={2} />
                                </div>
    ...
    return (
        <>
            ...
            <Model isOpen={isDeleteModelOpen}
                onClose={() => setIsDeleteModelOpen(false)}
                title='Delete Card Set?'
            >
                <div className='space-y-6'>
                    <p className='text-sm text-slate-600'>
                        Are you sure you want to delete this card set? This action cannot be undone and all cards will be permanently removed.
                    </p>
