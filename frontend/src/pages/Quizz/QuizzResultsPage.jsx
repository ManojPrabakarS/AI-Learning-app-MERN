...
const QuizzResultsPage = () => {
  ...
  const getScoreColor = (score) => {
    if (score >= 80) return 'from-purple-500 to-violet-500';
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="mb-6">
        <Link to={`/documents/${quiz.document._id}`}
          className="group inline-flex items-center gap-2 text-sm font-medium text-slate-600 hover:text-purple-600 transition-colors duration-200">
      ...
      {/* Score Card */}
      <div className="bg-white/80 backdrop-blur-xl border-xl border-2 border-slate-200 rounded-2xl shadow-xl shadow-slate-200/50 p-8 mb-8">
        <div className="text-center space-y-6">
          <div className="inline-flex items-center justify-center w-15 h-15 rounded-2xl bg-linear-to-br from-purple-100 to-violet-100 shadow-purple-500/25">
          ...
          <div className="flex items-center justify-center gap-4 pt-4">
            ...
            <div className="flex items-center gap-2 px-4 py-2 bg-purple-50 border border-purple-200 rounded-xl">
              <CheckCircle2 className="w-4 h-4 text-purple-600" strokeWidth={2} />
      ...
      <div className="space-y-6">
        ...
        {detailedResults.map((results, index) => {
          ...
          return (
            <div
              key={index}
              className="bg-white/80 backdrop-blur-xl border-2 border-slate-200 rounded-2xl p-6 shadow-slate-200/50"
            >
              <div className="flex items-start justify-between gap-4 mb-3">
                ...
                <div className={`shrink-0 w-10 h-10 rounded-xl flex items-center justify-center ${isCorrect ? 'bg-purple-50 border-2 border-purple-200' : 'bg-rose-50 border-2 border-rose-200'
                  }`}>
                  {isCorrect ? (
              ...
              <div className="">
                {result.options.map((option, optIndex) => {
                  ...
                  return (
                    <div
                      key={optIndex}
                      className={`relative px-4 py-3 rounded-lg border-2 transition-all duration-200 ${isCorrectOption ? 'bg-purple-50 border-purple-300 shadow-lg shadow-purple-500/10'
                        : isWrongAnswer ? 'bg-rose-50 border-rose-300' : 'bg-slate-50 border-slate-200'
                        }`}
                    >
                      <div className="">
                        <span className={`text-sm font-medium ${isCorrectOption ? 'text-purple-900' : isWrongAnswer ? 'text-rose-900' : 'text-slate-700'
                          }`}>
                          {option}
                        </span>
