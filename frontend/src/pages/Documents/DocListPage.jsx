...
const DocListPage = () => {
  ...
  const fetchDoc = async () => {
    try {
      ...
      toast.error("Failed to fetch documents.. Try again..!")
      console.error(error)
    } finally {
      setLoading(false)
    }
      ...
      return (
        <div className="flex items-center justify-center min-h-[400px]">
          <div className="text-center max-w-md">
            ...
            <button onClick={() => setIsUploadModelOpen(true)} className="inline-flex items-center gap-2 px-6 py-3 bg-linear-to-r from-purple-500 to-violet-500 hover:from-purple-600 hover:to-purple-600 text-white text-sm font-semibold rounded-xl transition-all duration-200 shadow-lg shadow-purple-500/25 hover:shadow-xl hover:shadow-purple-500/30 active:scale-[0.98]">
  ...
  return (
    <div className="min-h-screen">
          ...
          <form onSubmit={handleUpload} className="space-y-5">
            ...
            <div className="space-y-2">
              ...
              <input type="text"
                ...
                required
                className="w-full h-12 px-4 border-2 border-slate-300 rounded-xl bg-slate-50/50 text-slate-900 placeholder-slate-400 text-sm  font-medium transition-all duration-200 focus:outline-none focus:border-purple-500 focus:bg-white focus:shadow-lg focus:shadow-purple-500/10"
            ...
            <div className="space-y-2">
              ...
              <div className="relative border-2 border-dashed border-slate-500 rounded-xl bg-slate-50/50 hover:border-purple-500 hover:bg-purple-50/30 transition-all duration-200">
                <input type="file"
                  id='file-upload'
                ...
                <div className="flex flex-col items-center justify-center py-10 px-6">
                  <div className="w-14 h-14 rounded-xl bg-linear-to-r from-purple-200 to-violet-200 flex items-center justify-center mb-4">
                    <Upload className="w-7 h-7 text-purple-600" strokeWidth={2} />
                  </div>
                  ...
                  <p className="text-xs text-slate-500">PDF Size upto 10MB</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex gap-3 pt-2">
              <button
                ...
                onClick={() => setIsUploadModelOpen(false)}
                disabled={uploading}
                className="flex-1 h-11 px-4 border-2 border-slate-200 rounded-xl bg-white text-slate-700 text-sm font-semibold hover:bg-slate-50 hover:border-slate-400 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Cancel
              </button>
              ...
              <button
                type="submit"
                disabled={uploading}
                className="flex-1 h-11 px-4 bg-linear-to-r from-purple-500 to-violet-500 hover:from-purple-600 hover:to-violet-600 text-white text-sm font-semibold rounded-xl transition-all duration-200 shadow-lg shadow-purple-500/25 disabled:opacity-50 disabled:cursor-not-allowed active:scale-[0.98]"
              >
                {uploading ? (
      ...
      {isDeleteModelOpen && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
